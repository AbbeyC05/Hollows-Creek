...
function theatreBusinessCard(){
 ...
 let grid=page.querySelector('.biz-grid')||page.querySelector('.business-grid')||page.querySelector('.bw-grid')||page.querySelector('.directory');
 if(!grid){
   let section=document.getElementById('beaumont-theatre-directory-section');
   if(!section){section=document.createElement('section');section.id='beaumont-theatre-directory-section';section.className='bw-section';section.innerHTML='<div class="bw-kicker">ENTERTAINMENT</div><h2>Beaumont Theatre</h2><div class="biz-grid"></div>';page.appendChild(section)}
   grid=section.querySelector('.biz-grid');
 }
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(run,2500));else setTimeout(run,2500);
[4500,7000,10000,14000].forEach(t=>setTimeout(run,t));
})();